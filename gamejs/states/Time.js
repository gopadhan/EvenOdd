...
this._questionBox = null;
this._questionBoxText = null;
...
createQuestion: function(){
            ...
            this._question = 'Identify the clock displaying the correct Time.'
            ...
            this._questionBox = this.add.graphics(0, 0);
            
            // set a fill and line style for the circle
            this._questionBox.lineStyle(8, '#2d2d2d', 0.2);
            this._questionBox.beginFill(0xa000f3, 0.8);
        
            //drawRoundedRect(x, y, width, height, radius)
            this._questionBox.drawRoundedRect(vGame.world.centerX, 100, 100, 50, option._rectRadius);
            //this._questionBox.anchor.set(0.5);
            this._questionBox.endFill();

			this._questionBoxText = new Phaser.Text(vGame, 0, 0, addtime(clock._Hour, clock._Minute, 0, 0), textStyle);
			
			// graphics and textElement bounds/sizes must be the same
			// so your text area covers the whole circle
            this._questionBoxText.setTextBounds(vGame.world.centerX+50, 125);
            
			this._questionBox.addChild(this._questionBoxText);
			vGame.world.addChild(this._questionBox);            
...
createOptionSet: function(){
        ...
        this._optionsVal.sort(function(a, b){return 0.5 - Math.random()});
            ...
            //drawRoundedRect(x, y, width, height, radius)
            graphics.drawRoundedRect(10, 200, 770, 200, 15);
            graphics.anchor.set(0.5);
            graphics.endFill();
        
            option._group.add(graphics);
            vGame.world.addChild(option._group);
            // set a fill and line style for the rectangle
            ...
            for (var i =0 ; i < 4; i ++) {
                var fields = this._optionsVal[i].split(':');
                var clocksprite = this.createClock(clock._positionX + 475*i, clock._positionY, fields[0], fields[1], clock._Group, 0.4);
                //option._group.add(this._optionsBox[i]);
                
                clocksprite.name = this._optionsVal[i];
                clocksprite.inputEnabled = true;
                clocksprite.input.useHandCursor = true;
                clocksprite.events.onInputOver.add(this.mouseover, this);
                clocksprite.events.onInputOut.add(this.mouseout, this);
                clocksprite.events.onInputDown.add(this.clickAnswer, this);
            ...
            vGame.world.bringToTop(clock._Group);
